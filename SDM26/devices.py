    ...
    for device in devices:
        ...
        match name:
            ...
            case "r_Yaw":
                ...
                device.conversion_factor = lambda v: (v * 17.50) / 1000
                device.units = "deg/s"
                device.display_name = "r_Yaw"
                device.short_name = "r_Yaw"
            case "FR_SG":
                device.conversion_factor = lambda v: (v)
                device.units = "raw"
                device.display_name = "f_FR_PartTBD"
                device.short_name = "f_FR_PartTBD"
            case "FL_SG":
                device.conversion_factor = lambda v: (-11052026.1 * v + 2606.22253)
                device.units = ""
                device.display_name = "f_FL_PartTBD"
                device.short_name = "f_FL_PartTBD"
            case "RL_SG":
                device.conversion_factor = lambda v: (-1401922.44 * v + 92026.0137)
                device.units = ""
                device.display_name = "f_RL_PartTBD"
                device.short_name = "f_RL_PartTBD"
            case "RR_SG":
                device.conversion_factor = lambda v: (v)
                device.units = ""
            ...
            case "RESET_REASON":
                # esp_reset_reason_t value captured at boot (see esp_system.h).
                # CSV-only diagnostic channel; excluded from MoTeC export in conversion_pipeline.py.
                device.units = ""
                device.display_name = "Reset Reason"
                device.short_name = "RstRsn"
            case "DTC_FLW":
                device.units = ""
